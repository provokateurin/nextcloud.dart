...
for spec in /tmp/nextcloud-neon/*.openapi.json; do
  ...
  mkdir -p "lib/src/api/$name"
  cp "$spec" "lib/src/api/$name/"
done
...
jq \
  ...
  > lib/src/api/core/core.openapi.json

for patch in "lib/src/api/core/patches/"*; do
  jsonpatch --indent 4 --in-place lib/src/api/core/core.openapi.json "$patch"
done

(
  dart run scripts/generate_support_table.dart
  dart run scripts/generate_props.dart
  dart run build_runner build --delete-conflicting-outputs
  dart run scripts/generate_exports.dart
